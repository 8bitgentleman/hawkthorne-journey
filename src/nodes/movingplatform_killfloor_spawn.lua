...
function MovingPlatform.new(node, collider)
    ...
    mp.proximity = node.properties.proximity ~= 'false'
...
function MovingPlatform:update(dt,player)
   ...
   --experimental proximity 
    if self.singleuse and self.proximity and self.pos >= 1 then
        if math.abs(player.position.x - self.node.x) <= 100 and math.abs(player.position.y - self.node.y) <= self.y_Proximity + 0 and self.velocity.x < 0 then
            self.velocity.y = math.min(self.velocity.y + game.friction * dt, 0)
        else
            self.velocity.y = math.max(self.velocity.y - game.friction * dt, 0)
        end
        
        self.velocity.x = self.velocity.x + ( game.gravity / 2 ) * dt
