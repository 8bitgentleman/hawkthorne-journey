...
function HiddenDoorTrigger.new(node, collider)
    ...
    assert( node.properties.target2, 'You must provide a target2 property for the hiddendoortrigger node' )
    assert( node.properties.target3, 'You must provide a target3 property for the hiddendoortrigger node' )
    ...
    art.target2 = node.properties.target2
    art.target3 = node.properties.target3
...
function HiddenDoorTrigger:enter()
    ...
    Gamestate.currentState().doors[self.target2].node:hide()
    Gamestate.currentState().doors[self.target3].node:hide()
end
    ...
    if button == 'INTERACT' and self.prompt == nil then
        ...
        self.prompt = Prompt.new(self.message, function(result)
            if result == 'Yes' then
              ...
              Gamestate.currentState().doors[self.target2].node:show()
              Gamestate.currentState().doors[self.target3].node:show()
            end
            player.freeze = false
