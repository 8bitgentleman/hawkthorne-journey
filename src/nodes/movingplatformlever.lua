...
function MovingPlatform.new(node, collider)
    ...
    mp.triggerable = node.properties.triggerable ~= 'true'
...
-- generic support for triggerable platforms
    if mp.triggerable then
        mp.stopped = true
        mp.movetime = node.properties.movetime and tonumber(node.properties.movetime) or 1
...
-- everything below this is required for triggerable platforms
function MovingPlatform:moving()
    if self.triggerable and self.stopped then
        self.stopped = false
        sound.playSfx( 'movingstone' )
    end
end

function MovingPlatform:stop()
    if self.triggerable and not self.stopped then
        self.stopped = true
        sound.playSfx( 'movingstone' )
    end
end
