...
|---|---|---|---|---|---|
| **underwater2** | — (never PR'd) | no upstream thread | 144f/+2715 (core Lua small; `npc_old.lua` cruft) | ✅ **PORTED 2026-07-21** (`58a4857b`) | Oxygen/suffocation mechanic + jellyfish/bubbles enemies + `forest-underwater.tmx` reimplemented clean on develop (99 passed, lint clean). Floaty/buoyancy movement added `2026-07-21` (`08dd5fc2`). **DONE — no follow-ups. ⛔ Do NOT add a door entrance: owner's decision (2026-07-21) is that `forest-underwater` stays a test-only level, reachable via the scenario harness, never wired into normal play. Stop proposing this.** |
...
| **bomb** | — | no thread | 8f/+210 (2 PNGs) | **DEFERRED** (owner, 2026-07-21) | Player-throwable bomb; `projectile.lua` hook is oddly near-clean vs today's tree and doesn't collide with existing enemy bombs. Reimplement `bomb_throwable`/`bomb_explosion` clean (strip debug `print()`s, fix `explosing` typo). Standalone combat variety, no pillar tie. **Not applicable right now — parked, not archived; revisit if combat variety is wanted.** |
...
| **moneymoneymoney** | — | no thread | 30f/+222 (3 token PNGs) | **DEFERRED** (owner, 2026-07-21) | Tiered currency (`gold`/`greaterCoin`/`jewel`) + difficulty-scaled loot — develop's `tokens/` has only coin/health, so unclaimed. Concept + token art are salvageable; the drop-weight formula is half-baked WIP ("begin dependent drops", debug prints). Reimplement the concept, not the math. **Not applicable right now — parked, not archived.** |
...
| **leaderboard** | — | no thread | 4f/+283 | **DEFERRED** (owner, 2026-07-21, after firsthand re-read) | ⚠️ **Weaker than the original one-liner implied — it's a feature, not a salvage.** Two disconnected halves: (1) the highscore UI (`highscore_entry.lua`/`highscore_list.lua`, commit `e1b9647e`) is **verbatim copy from "Mr. Rescue"** — bare-global function tables + undefined globals (`highscores`/`level`/`score`/`STATE_HIGHSCORE_ENTRY`/`WIDTH`/`updateKeys()`), wrong architecture, unrunnable; (2) the stats sliver (`21871476`) adds `plyr.deaths`/`plyr.time` + `Player:round()` but is WIP — `deaths` incremented oddly in `refreshPlayer`, a leftover debug `love.graphics.print` timer in `Player:draw()`, and a `welcome.lua` "highscores" menu item that just routes to `instructions`. **The halves don't connect: the UI wants a per-level `score` this game has no concept of, while the stats track deaths/time.** So there's no "take the stats, drop the UI" freebie — you'd be *designing* what a leaderboard measures here, then building UI from zero. Only liftable atom = deaths/playtime tracking, and only if a future stats/speedrun feature wants it. **Parked.** |
...
**Owner deferrals (2026-07-21):** `bomb`, `moneymoneymoney`, and `leaderboard` reclassified
SALVAGE-PARTS → **DEFERRED** (not applicable right now; kept on origin, not archived). `leaderboard`
in particular is a feature-not-a-salvage after re-read — see its row. Active SALVAGE-PARTS is now 8.
...
**Remaining follow-up:** none. ⛔ **Door entrance was intentionally NOT added and never will be** —
owner's decision (2026-07-21): `forest-underwater` is a **test-only level**, reached via the
scenario harness, deliberately unreachable in normal play. It is invisible to `test_maps.lua`'s
door walk by design. **Do not re-raise wiring a door — this is settled.**
